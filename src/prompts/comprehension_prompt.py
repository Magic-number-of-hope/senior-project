...
5. 当用户未输入起点时，先判断是否可由系统补全：
  - 如果语义明显指向当前位置（如"我现在""附近""离我最近"），可保持 `origin=null`，由系统优先尝试使用当前位置补全
  - 如果语义更像延续上一轮路线（如"再去""然后去"），可保持 `origin=null`，由系统尝试用上下文补全
6. 尽可能提取所有槽位，缺失的留空即可（系统会自动向用户索要缺失信息）
6. 只有在语义模糊（如多种理解方式）时才设置 needs_clarification=true

## 必要槽位说明
以下三个槽位是导航必须的，如果用户未提供则填 `null`（不要自己编造默认值）：
- **origin**：出发地（用户没说"从哪出发"就填 `null`，不要编造具体地名；系统会尝试用当前位置或上下文补全）
- **destination**：目的地
- **travel_mode**：出行方式(driving/walking/transit/bicycling)（用户没提到出行方式就填 `null`）
...
注意：origin/destination/travel_mode 缺失时必须输出 `null`，不要填默认值；其中 origin 可能由系统基于当前位置或上下文自动补全。
"""
