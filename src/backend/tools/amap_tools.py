...
def _looks_like_precise_address(address: str) -> bool:
    """判断输入是否更像精确地址，而不是泛化 POI 名称。"""
    addr = (address or "").strip()
    if not addr:
        return False
    ...
    hit_count = sum(1 for token in address_tokens if token in addr)
    return hit_count >= 2
...
def _city_matches_text(text: str, city: str) -> bool:
    """判断返回地址是否仍落在给定城市上下文内。"""
    ...
    if not target_text:
        return False
    return city_token in target_text or f"{city_token}市" in target_text
...
async def geocode(address: str, city: str = "") -> ToolResponse:
    ...
    # 对泛化 POI 名称且没有城市约束的输入，宁可返回 no_result，避免跨城误命中。
    if not city and not _looks_like_precise_address(address):
        return _text_response(json.dumps({
            "status": "no_result",
            "api_info": poi_data.get("info", ""),
        }, ensure_ascii=False))
    ...
    if data.get("status") == "1" and data.get("geocodes"):
        ...
        if city and not _city_matches_text(formatted_address, city):
            return _text_response(json.dumps({
                "status": "no_result",
                "api_info": f"city_mismatch:{formatted_address}",
            }, ensure_ascii=False))
        return _text_response(json.dumps({
            "status": "ok",
