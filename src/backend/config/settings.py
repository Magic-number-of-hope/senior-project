...
# ── 前端接入配置 ──
# 开发模式建议显式指向 Vue dev server（例如 http://localhost:8080）。
# 默认置空，后端将优先提供构建后的前端页面，避免直接运行后端时跳到未启动的 dev server。
FRONTEND_DEV_URL = os.getenv("FRONTEND_DEV_URL", "")

# 前端构建目录（用于未启用 dev server 时的回退）。
_BACKEND_DIR = os.path.dirname(os.path.dirname(__file__))
FRONTEND_DIST_DIR = os.getenv(
    "FRONTEND_DIST_DIR",
    os.path.join(_BACKEND_DIR, "..", "front", "dist"),
)
